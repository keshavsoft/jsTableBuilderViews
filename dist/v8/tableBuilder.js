...
function F(e, t) {
	...
	})) : e.dataStore.data = [...e.dataStore.originalData], P(e);
}
//#endregion
//#region buildTable/buildTopHeader.js
function I({ inLabel: e = "", inPlaceholder: t = "", inClasses: n = {}, inOnSearch: r = () => {} }) {
...
function R({ inTableOptions: e = {} }) {
	let t = {
		inTableWidth: M.tableOptions.commonOptions.tableWidth,
		inTableBorder: M.tableOptions.commonOptions.tableBorder,
		inShowSerialNo: M.tableOptions.commonOptions.showSerialNo
	}, n = { inHeaderHeight: M.tableOptions.headOptions.headerHeight }, r = { inRowHeight: M.tableOptions.bodyOptions.rowHeight }, i = {
		inShowFooter: M.tableOptions.footOptions.showFooter,
		inRowHeight: M.tableOptions.footOptions.rowHeight
	}, a = {
		...t,
		...e.inCommonOptions || {}
	}, o = {
	...
	return {
		inCommonOptions: {
			inTableWidth: L(a?.inTableWidth),
			inTableBorder: L(a?.inTableBorder),
			inShowSerialNo: a?.inShowSerialNo
		},
		inHeadOptions: { inHeaderHeight: L(o?.inHeaderHeight) },
		inBodyOptions: { inRowHeight: L(s?.inRowHeight) },
		inFootOptions: {
			inShowFooter: c?.inShowFooter,
			inRowHeight: L(c?.inRowHeight)
		}
	};
}
//#endregion
...
function B({ inTopHeader: e }) {
	return e === M.topHeader ? {
		inShow: M.topHeader.show,
		inLabel: M.topHeader.label,
		inPlaceholder: M.topHeader.placeholder
	} : {
		inShow: e.show === void 0 || e.show,
		inLabel: e.label === void 0 ? M.topHeader.label : e.label,
		inPlaceholder: e.placeholder === void 0 ? M.topHeader.placeholder : e.placeholder
	};
}
//#endregion
//#region buildTable/utils/config/mergeClasses.js
function V({ inClasses: e, inTheme: t = "style1" }) {
	let n = e || {}, r = j[t] || j.style1;
	return {
		...r,
		...n,
		head: {
...
}, W = ({ inColumns: e, inShowSerialNo: t }) => U(e, t).map((e) => {
	let t = { ...e };
	return t.options && t.options.width && (t.options = {
		...t.options,
		width: L(t.options.width)
	}), t;
}), G = ({ inData: e, inShowSerialNo: t }) => {
...
var q = {
	...
	create: async (e, t) => K(e, {
		method: "POST",
		body: JSON.stringify(t)
	}),
	update: async (e, t) => K(e, {
		method: "PUT",
		body: JSON.stringify(t)
	}),
	delete: async (e, t) => K(`${e}/${t}`, { method: "DELETE" })
};
function J(e, t = {}) {
	e.endPoints = t, e.services = {};
	try {
		Object.keys(q).forEach((n) => {
			t[n] && (e.services[n] = (...e) => q[n](t[n], ...e));
		});
	} catch (e) {
...
var Y = ({ instance: e, localColumns: t, localData: n, localEndPoints: r }) => {
	e.dataStore.columns = W({
		inColumns: t,
		inShowSerialNo: e.tableOptions?.inCommonOptions?.inShowSerialNo
	}), e.dataStore.originalData = n, r ? J(e, r) : e.dataStore.data = G({
		inData: n,
		inShowSerialNo: e.tableOptions?.inCommonOptions?.inShowSerialNo
	});
}, X = class {
	constructor({ htmlId: e, data: t, columns: n = [], classes: r = {}, theme: i = "style1", tableOptions: a = {}, topHeader: o = M.topHeader, endPoints: s }) {
		let c = e, l = t, u = n, d = r, f = s, p = z(a);
		this.tableOptions = R({ inTableOptions: p }), this.topHeader = B({ inTopHeader: o }), this.htmlId = c, this.dataStore = {}, Y({
			instance: this,
			localColumns: u,
			localData: l,
			localEndPoints: f
	...
	async appendToDom() {
		this.dataStore.originalData = await this.services.read(), this.dataStore.data = G({
	...
	buildTopHeaderElement() {
		return !this.topHeader || this.topHeader.inShow === !1 ? null : I({
	...
	inTableOptions: z(M.tableOptions),
	inTopHeader: {
		inShow: M.topHeader.show,
		inLabel: M.topHeader.label,
		inPlaceholder: M.topHeader.placeholder
...
window.ks = window.ks || {}, window.ks.TableBuilder = X, window.ks.TableBuilder.DEFAULT_CLASSES = j, window.ks.TableBuilder.DEFAULT_CONFIG = M, window.ks.TableBuilder.DEFAULT_INTERNAL_OBJECT = Z, window.ks.TableBuilder.version = "v8.0";
//#endregion
export { j as DEFAULT_CLASSES, M as DEFAULT_CONFIG, Z as DEFAULT_INTERNAL_OBJECT, X as TableBuilder };
